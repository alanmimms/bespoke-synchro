  ...
  function synchro(deck) {
    ...
    if (synchro.sending) {
      ...
      deck.on('slide+bullet', function(e) {
        if (synch()) {return;}
        if (!control(url, socket).local) {return;}
        socket.send(e.slidePlusBullet.slideIndex + ',' + e.slidePlusBullet.bulletIndex);
      });
    }
    ...
    socket.onmessage = function (e) {
      ...
      deck.slide(+d[0]);
      deck.fire('slide+bullet', {slidePlusBullet: {slideIndex: +d[0], bulletIndex: +d[1]}});
    };
